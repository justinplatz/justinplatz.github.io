<!DOCTYPE html><html lang="en"><head>
  ...
  <script type="module">import '@polymer/iron-test-helpers/mock-interactions.js';
...
<test-fixture id="basicCarousel">
  <template>
    <skeleton-carousel dots="" nav="">
      <div>
        <iron-image data-src="http://source.unsplash.com/category/nature/400x400" sizing="cover" preload=""></iron-image>
      </div>
      <div>
        <iron-image data-src="http://source.unsplash.com/category/food/400x400" sizing="cover" preload=""></iron-image>
      </div>
    </skeleton-carousel>
  </template>
</test-fixture>
  ...
  <template>
    <div>
      <dom-bind>
        <template>
          <skeleton-carousel dots="" nav="">
            <template is="dom-repeat" items="[&quot;https://source.unsplash.com/category/buildings/500x300&quot;, &quot;https://source.unsplash.com/category/food/500x300&quot;]">
              <iron-image placeholder="https://source.unsplash.com/category/nature/10x10" data-src$="[[item]]" sizing="cover" preload="" fade=""></iron-image>
            </template>
          </skeleton-carousel>
        </template>
      </dom-bind>
...
<script type="module">import '@polymer/iron-image/iron-image.js';
import '../skeleton-carousel.js';
import { a11ySuite } from 'wct-browser-legacy/a11ySuite.js';

const oncePropChanged = (el, prop) => {
  return new Promise((resolve, reject) => {
    const dashedPropName = prop.replace(/[A-Z]/g, match => `-${match[0].toLowerCase()}`).concat('-changed');
...
suite('Disable option', () => {
  test('showPrev & showNext should be disable', done => {
    ...
    flush(() => {
      const buttons = element.shadowRoot.querySelector('#dots').querySelectorAll('paper-icon-button');
...
suite('Keyboard actions', () => {
  ...
  test('Should control prev and next slide with keybindings', done => {
    ...
    flush(() => {
      ...
      expect(element.selected).to.equal(1); // Press left arrow key

      MockInteractions.pressAndReleaseKeyOn(element, keyCodes.left);
      expect(element.selected).to.equal(0); // Press down arrow key

      MockInteractions.pressAndReleaseKeyOn(element, keyCodes.down);
      expect(element.selected).to.equal(1); // Press up arrow key
