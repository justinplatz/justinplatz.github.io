...
class SkeletonCarousel extends mixinBehaviors([IronA11yKeysBehavior, GestureEventListeners], PolymerElement) {
  /**
   * Template element
...
`;
  ...
  connectedCallback() {
    ...
    const container = this.shadowRoot.querySelector('slot'); // Allow vertical scrolling
    ...
    new FlattenedNodesObserver(container, () => {
      this._children = FlattenedNodesObserver.getFlattenedNodes(this).filter(n => n.nodeType === Node.ELEMENT_NODE && n.nodeName !== 'DOM-REPEAT' && n.nodeName !== 'TEMPLATE');
  ...
  static get observers() {
    return ['_translateObserver(direction, selected, _x, _y)'];
  }
  /**
   * Key bindings
   * @return {object}
  ...
  _lazyContent(selected) {
    ...
    if (!child) return; // lazy load first level content
  ...
  _drag(event) {
    ...
    let finalPositionTop = Number(finalTop); // Prevent drag to the wrong direction if is not available
