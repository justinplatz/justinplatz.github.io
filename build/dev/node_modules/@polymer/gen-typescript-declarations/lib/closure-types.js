  ...
  switch (node.type) {
    ...
    case 'RestType':
      return new ts.ParamType({
        ...
        type: new ts.ArrayType(node.expression !== undefined ? convert(node.expression, templateTypes) : ts.anyType),
        optional: false,
        rest: true
      });
...
function convert(node, templateTypes) {
  ...
  if (isParameterizedArray(node)) {
    ...
    // null
    t = ts.nullType;
  } else if (isUndefinedLiteral(node)) {
    // undefined
    t = ts.undefinedType;
...
const renameMap = new Map([// Closure's `Object` type excludes primitives, so it is closest to
// TypeScript's `object`. (Technically this should be `object|Symbol`, but we
// will concede that technicality.)
['Object', 'object'], // The tagged template literal function argument.
['ITemplateArray', 'TemplateStringsArray']]);
/*
 * As above but only applicable when parameterized (`Foo<T>`)
 */

const parameterizedRenameMap = new Map([['HTMLCollection', 'HTMLCollectionOf'], ['NodeList', 'NodeListOf']]);
/**
...
function convertArray(node, templateTypes) {
  ...
  return new ts.ArrayType(applications.length === 1 ? convert(applications[0], templateTypes) : ts.anyType);
}
...
function convertParameterizedType(node, templateTypes) {
  ...
  const name = renameMap.get(node.expression.name) || parameterizedRenameMap.get(node.expression.name) || node.expression.name;
...
function convertFunction(node, templateTypes) {
  const params = node.params.map((param, idx) => convertParam( // TypeScript wants named parameters, but we don't have names.
  'p' + idx, param, templateTypes));

  if (node.new) {
    return new ts.ConstructorType(params, // It doesn't make sense for a constructor to return something other
    ...
    return new ts.FunctionType(params, // Cast because type is wrong: `FunctionType.result` is not an array.
...
function convertRecord(node, templateTypes) {
  ...
  for (const field of node.fields) {
    ...
    const fieldType = field.value ? convert(field.value, templateTypes) : ts.anyType; // In Closure you can't declare a record field optional, instead you
...
function isParameterizedArray(node) {
  return node.type === 'TypeApplication' && node.expression.type === 'NameExpression' && node.expression.name === 'Array';
}
...
function isParameterizedObject(node) {
  return node.type === 'TypeApplication' && node.expression.type === 'NameExpression' && node.expression.name === 'Object';
}
