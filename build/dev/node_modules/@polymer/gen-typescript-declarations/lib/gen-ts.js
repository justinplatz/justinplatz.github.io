...
const defaultExclude = ['index.html', 'test/**', 'demo/**'];
/**
...
function analyzerToAst(analysis, config, rootDir) {
  return __awaiter(this, void 0, void 0, function* () {
    const excludeFiles = (config.excludeFiles || config.exclude || defaultExclude).map(p => new minimatch.Minimatch(p));
    ...
    const renameTypes = new Map(Object.entries(config.renameTypes || {})); // Map from identifier to the module path that exports it.
    ...
    const analyzerDocs = [...analysis.getFeatures({
      kind: 'html-document'
    ...
    for (const [declarationsFilename, analyzerDocs] of declarationDocs) {
      const tsDoc = new ts.Document({
        ...
        header: makeHeader(analyzerDocs.map(d => analyzerUrlToRelativePath(d.url, rootDir)).filter(url => url !== undefined)),
        tsLintDisables: [{
      ...
      tsDoc.simplify(); // Include even documents with no members. They might be dependencies of
    ...
    const filteredWarnings = warnings.filter(warning => {
      ...
      return sourcePath !== undefined && !excludeFiles.some(pattern => pattern.match(sourcePath));
    });
  ...
  for (const node of tsDoc.traverse()) {
    if (node.kind === 'name') {
      ...
      if (importSpecifier.startsWith('.')) {
        ...
        importSpecifier = path.relative(path.dirname(tsDoc.path), importSpecifier);
...
function googModuleForNameBasedImportSpecifier(spec) {
  const name = // remove trailing .d.ts and .js
  spec.replace(/(\.d\.ts|\.js)$/, '') // foo-bar.dom becomes fooBarDom
  .replace(/[-\.](\w)/g, (_, s) => s.toUpperCase()) // remove leading @
  .replace(/^@/g, '') // slash separated paths becomes dot separated namespace
  .replace(/\//g, '.'); // add goog: at the beginning
...
function transformToGoogStyle(tsDoc, rootDir) {
  ...
  for (const child of tsDoc.traverse()) {
    if (child.kind === 'import' || child.kind === 'export') {
      ...
      if (spec.startsWith('.')) {
        spec = path.join(packageName, path.relative(rootDir, path.join(rootDir, path.dirname(tsDoc.path), spec)).replace(/^\.\//, ''));
      }
      ...
      if (elementName && !/\./.test(elementName)) {
        trailingComment = ` // from //third_party/javascript/polymer/v2/${elementName}`;
      }
...
class TypeGenerator {
  ...
  handleDocument() {
    for (const feature of this.analyzerDoc.getFeatures()) {
      ...
      if (feature.kinds.has('element')) {
        ...
        // hence this check.
        if (feature.sourceRange && feature.sourceRange.file === this.analyzerDoc.url) {
  ...
  handleElement(feature) {
    ...
    if (constructable) {
      ...
      parent.members.push(new ts.Interface({
        ...
        extends: [...feature.mixins.map(mixin => mixin.identifier), ...legacyPolymerInterfaces]
      }));

      if (isPolymerElement(feature) && feature.isLegacyFactoryCall && this.root.isEsModule) {
    ...
    if (feature.tagName) {
      const elementMap = findOrCreateInterface(this.root.isEsModule ? findOrCreateGlobalNamespace(this.root) : this.root, 'HTMLElementTagNameMap');
  ...
  handleBehavior(feature) {
    ...
    const ns = findOrCreateNamespace(this.root, namespacePath); // An interface with the properties and methods that this behavior adds to
  ...
  handleMixin(feature) {
    ...
    const constructorName = mixinName + 'Constructor'; // The mixin function. It takes a constructor, and returns an intersection
    ...
    parentNamespace.members.push(new ts.Function({
      ...
      params: [new ts.ParamType({
        name: 'base',
        type: new ts.NameType('T')
      })],
      returns: new ts.IntersectionType([new ts.NameType('T'), new ts.NameType(constructorName), ...transitiveMixins.map(mixin => new ts.NameType(mixin.name + 'Constructor'))])
    }));
    ...
    if (this.root.isEsModule) {
      ...
      for (const mixin of transitiveMixins) {
        ...
        const identifiers = [{
          identifier: mixin.name + 'Constructor'
        }];
  ...
  transitiveMixins(parentMixin, result) {
    ...
    for (const childRef of parentMixin.mixins) {
      const childMixinSet = this.analysis.getFeatures({
        id: childRef.identifier,
        kind: 'element-mixin'
      });

      if (childMixinSet.size !== 1) {
        this.warn(parentMixin, `Found ${childMixinSet.size} features for mixin ` + `${childRef.identifier}, expected 1.`);
        continue;
      }
  ...
  handleClass(feature) {
    ...
    const m = new ts.Class({
      name
    });
    m.description = feature.description;
    m.properties = this.handleProperties(feature.properties.values());
    m.methods = [...this.handleMethods(feature.staticMethods.values(), {
      isStatic: true
    }), ...this.handleMethods(feature.methods.values())];
    m.constructorMethod = this.handleConstructorMethod(feature.constructorMethod);
  ...
  handleProperties(analyzerProperties) {
    ...
    for (const property of analyzerProperties) {
      if (property.inheritedFrom || property.privacy === 'private' || this.excludeIdentifiers.has(property.name)) {
        continue;
      }
  ...
  handleMethods(analyzerMethods, opts) {
    ...
    for (const method of analyzerMethods) {
      if (method.inheritedFrom || method.privacy === 'private' || this.excludeIdentifiers.has(method.name)) {
        continue;
      }
  ...
  documentationHasSuppressTypeCheck(annotation) {
    ...
    return annotationValue && annotationValue.description === '{checkTypes}' || false;
  }
  ...
  handleJsImport(feature) {
    ...
    } else if ( // Exports are handled as exports below. Analyzer also considers them
    // imports when they export from another module.
    !babel.isExportNamedDeclaration(node) && !babel.isExportAllDeclaration(node)) {
  ...
  handleJsExport(feature) {
    ...
    if (babel.isExportAllDeclaration(node)) {
      ...
      if (node.declaration) {
        ...
        for (const specifier of node.specifiers) {
          if (this.isResolvable(specifier.exported.name, feature) || this.isResolvable(specifier.local.name, feature)) {
            identifiers.push({
  ...
  isResolvable(identifier, fromFeature) {
    ...
    return resolved !== undefined && resolved.feature !== undefined && !isPrivate(resolved.feature) && !isBehaviorImpl(resolved);
  }
  /**
  ...
  handleHtmlImport(feature) {
    ...
    sourcePath = sourcePath.replace(/^(bower_components|node_modules)\//, '../'); // Polymer is a special case where types are output to the "types/"
...
function isBehaviorImpl(reference) {
  return reference.feature !== undefined && reference.feature.kinds.has('behavior') && reference.feature.name !== reference.identifier;
}
/**
